...
#include "my_global_planner/my_global_planner.hpp" // Include the custom global planner header

namespace my_global_planner // Define the namespace for the planner
{
...
{
  RCLCPP_INFO(
    node_->get_logger(), "CleaningUp plugin %s of type MyGlobalPlanner", // Log the cleanup process
    name_.c_str());
}
...
{
  RCLCPP_INFO(
    node_->get_logger(), "Activating plugin %s of type MyGlobalPlanner", // Log the activation process
    name_.c_str());
}
...
{
  RCLCPP_INFO(
    node_->get_logger(), "Deactivating plugin %s of type MyGlobalPlanner", // Log the deactivation process
    name_.c_str());
}
...
PLUGINLIB_EXPORT_CLASS(my_global_planner::MyGlobalPlanner, nav2_core::GlobalPlanner)
